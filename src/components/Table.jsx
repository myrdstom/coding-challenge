...
import {Table, TableBody, TextField, Grid, Container} from "@material-ui/core";
...
import { MinusSign, PlusSign, RightCaret, DownCaret, ScorecardIcon, SearchIcon, NoteIcon } from "../ui-kit/icons";
...
export default function CustomizedTables() {
  ...
  return (
    <Container className={classes.root}>
      ...
      <Grid container>
        <Grid item xs={10}>
          <TextField className={classes.textField} id="filled-error-helper-text" placeholder="Search by Candidate Or Keyword" variant="outlined" />
        </Grid>
        <Grid item xs={2}>
                ...
                <StyledTableRow key={candidate.id}>
                  ...
                  <StyledTableCell align="right">
                    {candidate.id === selected ? (
                      <MinusSign onClick={() => setSelected(candidate.id === selected ? null : candidate.id)} className={classes.cusrsor}/>
                    ) : (
                      <PlusSign onClick={() => setSelected(candidate.id === selected ? null : candidate.id)} className={classes.cusrsor}/>
                    )}
                  </StyledTableCell>
                </StyledTableRow>
                {candidate.id === selected && (
                  <div className={classes.accordion}>
                    {candidate.applications.map(application => (
                      <Grid container spacing={3} onClick={() => handleClickApplication({ ...candidate, application })} className={classes.gridContainerAccordion}>
                        <Grid item xs={4}><span>{application.role.title}</span></Grid>
                        <Grid item xs={4}><span>{application.new_status.label}</span></Grid>
                        <Grid item xs={4}><RightCaret /></Grid>
                      </Grid>
                    ))}
                  </div>
                )}
